...
def create_prompt(question, wbdata_context):
    wbdata_part = wbdata_context.strip() if wbdata_context else ""
    if not wbdata_part:
        return f"Tu es un assistant économique et touristique.\nQuestion : {question}\nJe ne dispose pas de cette information."
    return f"""
Tu es un assistant économique et touristique intelligent.
Question : {question}
{wbdata_part}
Réponds uniquement sur les données disponibles.
"""
...
def save_conversation(question, response, file_path="logs/conversation_log.json"):
    ...
    response_text = response.content if hasattr(response, "content") else str(response)
    conversation = {"timestamp": datetime.now().isoformat(), "question": question, "response": response_text}
...
def run_chatbot(selected_Années=None, selected_specific_years=None, selected_specific_trimestre=None, page_data=None):
        """
    page_data : dataframe ou texte qui correspond aux données visibles sur cette page
    """
        if "chat_history" not in st.session_state:
            st.session_state.chat_history = []
